...
var bookmarklet = function() {
 ...
 // INSERT_GIF_JS_HERE
  ...
  var playGIF = function(gif) {
    ...
    var doPlay = (function() {
        ...
        var initToolbar = function() {
          ...
          var elt = function(tag, cls, attrs) {
            ...
            if (cls) e.className = 'gif-sound_' + cls;
            for (var k in attrs) {
              e[k] = attrs[k];
            }
            return e;
          ...
          var mute = elt('button', 'mute');
          var close = elt('button', 'close');


          var updateTools = function() {
            toolbar.style.visibility = 'visible'; // See TODO.

            mute.innerHTML = muted ? unmuteIcon : muteIcon;
              mute.title = muted ? 'mute' : 'un mute';
            close.innerHTML = closeIcon;
              close.title = 'Close sound-gif and go back to original image';
            ...
            if (frames.length < 2) { // XXX
              ...
              if (loadError == 'xhr') {
                ...
                popup.innerHTML ='&nearr;';
                  popup.title = 'Click to open GIF in new window; try running gif-sound there instead';
            ...
            // XXX Blach.
            var simpleToolList = [mute, close];
            populate(toolbar, [simpleTools]);
          ...
          var doToggleMute = function() {
            muted = !muted;
            sound_element.volume = muted ? 0 : 1;
            updateTools();
          };
          ...
          mute.addEventListener('click', doToggleMute, false);
          ...
          canvas.addEventListener('mouseout', doPause, false);
          canvas.addEventListener('mouseover', doPlay, false);
    ...
    div.className = 'gif-sound';
    toolbar.className = 'gif-sound_toolbar';
  ...
  var gifs = imgs.filter(function(img) {
    ...
    // see a good way of figuring out if an image is a GIF, though, sjsgo this
  ...
  var mkOverlay = function(gif) {
    if (elemClasses(gif).indexOf('gif-sound_overlaid') !== -1) return; // Idempotent.
    addClass(gif, 'gif-sound_overlaid');
    gif.addEventListener('click', clicked, false);
  };

  var rmOverlay = function(gif) {
    // XXX: What if the bookmarklet was run more than once?
    removeClass(gif, 'gif-sound_overlaid');
